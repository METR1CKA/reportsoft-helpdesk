<section>
  ...
  <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
    ...
    <div>
      ...
      <x-text-input id="active" name="active" type="text" class="mt-1 block w-full" :value="old('active', $user->active ? 'Activated' : 'Disabled')" required autofocus autocomplete="active" readonly />
    ...
    <div class="flex items-center gap-4">
      ...
      <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)" class="text-sm text-gray-600 dark:text-gray-400">{{ __('Saved.') }}</p>
      @endif
    </div>
  </form>
</section>
